    ...
    switch (currentView) {
      case ViewState.HOME:
        return (
          <>
            <div
              className="sticky top-0 z-10 bg-dag-dark/80 backdrop-blur-md border-b border-dag-border px-4 py-3 cursor-pointer"
              onClick={scrollToTop}
            >
      ...
      case ViewState.PROFILE:
        ...
        switch (profileTab) {
          case "POSTS":
            profilePosts = posts.filter(
              (p) => p.author.id === currentUser.id && !p.isReply
            );
            break;
          case "REPLIES":
            profilePosts = posts.filter(
          ...
          case "MEDIA":
            profilePosts = posts.filter(
              (p) => p.author.id === currentUser.id && !!p.imageUrl
            );
            break;
          case "LIKES":
            ...
            <div className="sticky top-0 z-10 bg-dag-dark/80 backdrop-blur-md border-b border-dag-border px-4 py-3">
              <h2 className="text-xl font-bold text-white flex items-center gap-1">
                ...
                {currentUser.isVerified && (
                  <ShieldCheck className="text-dag-accent" size={18} />
                )}
              </h2>
            ...
            <div className="relative">
              ...
              <div className="px-4 pb-4">
                ...
                <h1 className="font-bold text-2xl flex items-center gap-2">
                  {currentUser.username}
                  {currentUser.isVerified && (
                    <ShieldCheck className="text-dag-accent" size={24} />
                  )}
                </h1>
                ...
                <div
                  className="flex gap-4 mt-3 text-dag-accent text-sm font-mono bg-dag-card/50 p-2 rounded inline-block border border-dag-border cursor-pointer hover:bg-dag-card"
              ...
              <div className="border-b border-dag-border mt-2 overflow-x-auto no-scrollbar">
                <div className="flex min-w-max">
                  <button
                    ...
                    className={`flex-1 text-center py-4 px-6 font-medium hover:bg-white/5 cursor-pointer transition-colors relative ${
                      profileTab === "POSTS"
                  ...
                  >
                    Posts
                    {profileTab === "POSTS" && (
                      <div className="absolute bottom-0 left-0 right-0 h-1 bg-dag-accent rounded-t-full mx-auto w-10"></div>
                    )}
                  </button>
                  <button
                    ...
                    {!isWalletConnected ? (
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        {MOCK_NFTS.map((nft) => (
                          <div
                            key={nft.id}
                            className="bg-dag-card border border-dag-border rounded-xl overflow-hidden group hover:border-dag-accent/50 transition-all cursor-pointer"
                          >
    ...
    <div className="min-h-screen bg-dag-dark text-white max-w-[1400px] mx-auto flex relative">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-dag-dark/90 backdrop-blur-lg border-t border-dag-border flex justify-around py-3 z-50">
        <button
          ...
          className={
            currentView === ViewState.HOME ? "text-dag-accent" : "text-gray-500"
          }
          title="Home"
        >
          <Home
